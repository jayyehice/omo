...
const Header = (props) => {
    ...
    const navList = nav_list.map((obj, index) => {
        return (
            <li key={index} className="sm:flex sm:justify-between sm:items-center">
                <span className={`hidden sm:block mdi ${obj.icon} mr-1`}></span>
                <a
                    href={obj.url}
                    className=" bg-slate-100 sm:bg-transparent text-center block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-blue-700 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700"
                ><span className={`sm:hidden sm:block mdi ${obj.icon} mr-1`}></span>{obj.key}</a>
            </li>
        )
    });
        ...
        <header className="relative">
            <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
                <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                    ...
                    <span className="mdi mdi-menu text-3xl sm:hidden cursor-pointer" onClick={toggleSmNav}></span>
                    <div className={`${smNav} sm:hidden absolute right-0 top-0 z-10 w-full h-screen bg-neutral-400 opacity-50`}></div>
                    <ul className={`${smNav} sm:hidden absolute right-0 top-0 z-10 w-full`}>
                        {navList}
